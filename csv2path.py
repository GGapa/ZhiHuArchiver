...
def main() -> None:
    ...
    # 因为你目前没有 index.csv（旧数据），我们先跳过 legacy 部分
    # old_answer_path, old_article_path = load_legacy_paths("index.csv")
    
    combined_paths = sorted(
        set(paths), key=sort_key   # 只使用新下载的 CSV 数据
    )

    print("总路径数量:", len(combined_paths))
    with open("paths.json", "w", encoding="utf-8") as f:
        json.dump(combined_paths, f, indent=4, ensure_ascii=False)

    print("✅ paths.json 生成完成！")
